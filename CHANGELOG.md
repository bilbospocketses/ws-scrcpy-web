...
- **Velopack upgraded 1.2.0 → 1.2.158 across all touchpoints**: the npm SDK in `package.json`, the Rust crate in `Cargo.lock`, and the `vpk` CLI, which `scripts/vpk-path.mjs` derives from the npm SDK's resolved version. Two upstream changes reach this app's installs:
  ...
  - **Updates no longer leave a second launcher in the install root.** Our `--packTitle` (`ws-scrcpy-web`) differs from `--mainExe` (`ws-scrcpy-web-launcher.exe`). Under 1.2.0 that made each update write `ws-scrcpy-web-launcher.exe` beside the `ws-scrcpy-web.exe` the MSI installed. The package's stub is now `ws-scrcpy-web_ExecutionStub.exe`, so an update overwrites the one root launcher (velopack#985). Nothing in the app referenced the root launcher, since every internal path goes through `current\`, so no behaviour changes. A duplicate an earlier update already left behind stays on disk and is harmless.
...
- **Installing the Windows service no longer rolls back just because its first start was slow.** The first start of a freshly written `Servy.Service.CLI.exe` is cold (the next entry says exactly why), and that can take longer than SCM's default 30 s start timeout, and SCM then gives up (System log 7009/7000). The install made one start attempt, so `verifyServiceActive` found the service stopped and rolled the whole install back, although nothing was wrong with it. qa-harness caught this once in four runs on `v0.1.30-beta.124`, six minutes after a Defender platform update. `launcher/src/elevated_runner.rs` now retries the start once, 10 s after a failure, inside the same elevated run, so there's no second UAC prompt. A warm second start is typically fast. A service that is genuinely broken fails twice and rolls back as before. Both attempts' output is kept in the install log.

- **The first service start after an install is now fast, instead of racing SCM's 30 s timeout.** Measured on fresh Windows 11 guests, the slow part was never our app. Servy's service host is a single-file .NET program. The first time it runs, it unpacks about ninety files into LocalSystem's temp folder, and Microsoft Defender then does a cloud lookup for each DLL as it loads, taking 1.4 to 3 seconds each, one after another. About a dozen of them happen before the host can check in with SCM. On an idle guest that took 20 s. On a busy one it hit the 30 s limit and the install rolled back. No Defender update is needed for this to happen; a fresh machine is enough. After installing the service, the elevated helper now runs the host once on its own, pointed at the same folder the service will unpack into, and only then starts the service. That moves Defender's checks out of SCM's window. On a fresh guest the first start went from a 30.5 s timeout to 0.8 s. The warm-up is capped at 90 s and is best-effort. If it fails, the start retry above still stands behind it.
...
- **A device that cannot record the chosen audio format no longer loses its picture too.** Streaming asks the device for Opus audio by default, and on a device with no Opus recorder that request did not simply produce silence — it stopped the whole stream before the first frame, so the screen stayed black with nothing on screen to say why. The cause was entirely on the audio side, which is the last place anyone would look for a missing picture. The server now asks the device which audio formats it can actually record and picks one it has, falling back to AAC where Opus is missing, and only turning audio off when the device offers nothing usable at all — the picture is never given up for the sake of sound. Devices that do support Opus are unaffected and keep using it. A device that does not answer the question at all is left exactly as configured rather than being assumed to have no audio, so a quiet diagnostic cannot cost you working sound.

### Added

- **When streaming fails, the log now says where it failed.** A black screen had several possible causes that looked identical from outside — the device never described the video it was about to send, it described one and then sent nothing, it sent frames the server threw away because the browser had already gone, or the browser got everything and could not decode it. All four produced the same thing: a session that connects, a picture that never appears, and nothing written down. Anyone reporting it was asked for logs that could not have contained the answer. The server now records the exact settings it hands the device (so they can be compared against a working desktop `scrcpy` run), the video format and size the device replied with, when the first decodable data arrived, and — if nothing decodable arrives within eight seconds — a line naming which of those cases actually happened. Every session ends with a one-line summary whether or not anything went wrong, because a normal reading is what makes an abnormal one legible. Frames discarded because the browser had disconnected are now counted and reported instead of vanishing silently.
...
- **Local HTTPS (Settings → Server → Local HTTPS).** Streaming needs a secure browsing context, which plain `http://<lan-ip>:8000` never is — this is the wall issue #691 ran into. The new panel generates a certificate for this machine's LAN IP or a hostname you choose, using a vendored, locally-run `mkcert` fork with no elevation and nothing sent anywhere, and starts an HTTPS listener alongside the existing plain-HTTP one once that certificate exists. That fork is fetched on first use rather than at every boot, and it is verified before it is ever placed on disk: the release's checksum manifest is itself checked against a digest pinned in this repo's source, and the binary is then checked against that manifest — so a tampered release is caught by a value an attacker would also have to change here, in a diff. A failure at either step refuses the install outright rather than warning and continuing, and the two are reported differently, because a manifest that no longer matches its pin means the pin needs updating after a version bump, while a binary that no longer matches the manifest means a bad download. Installing the CA it hands you removes the browser's certificate warning; even without installing it, clicking through that warning is still a secure context, so streaming works from another machine either way. Plain HTTP can then be narrowed — refused or redirected for anything off this machine — with loopback always exempt from both, so Settings stays reachable even if a certificate goes bad. Changing that exposure mode takes effect immediately; enabling HTTPS for the first time, regenerating the certificate, or changing its port all need a restart, and the panel says which is which. This does not replace a real reverse proxy for anything beyond a home LAN — there is no ACME, no publicly trusted certificate, and the CA has to be installed by hand on every device that should stop warning. Revoking the certificate deletes the CA every device on the network has already been told to trust, and regenerating for a different address mints an entirely new one — both are one click away in the panel, and both mean re-installing the CA everywhere again.
...
- **A plain-HTTP port that cannot be bound no longer takes a working HTTPS listener down with it.** The server already degraded in one direction: if the HTTPS listener failed to bind, it said so and carried on serving plain HTTP. The reverse was still fatal, so a busy port 8000 — or an `EACCES` on any port below 1024, which the port field explicitly lets you choose — killed the whole process, including an HTTPS listener that had bound perfectly well a moment earlier. Both directions now behave the same way: the failure is logged with the port and the reason, and whatever else came up keeps serving. The one case worth a distinct message is when *neither* listener binds, because the app is then running and reachable on nothing at all, which is easy to mistake for healthy — that now says so in as many words, and names the remedy.
- **Two requests that start installing the same dependency at once now share a single install.** Double-clicking "generate" before mkcert is on disk could send two installs after the same destination file. They no longer collide over the same download and the same final rename; the second request waits for the first and gets its result. A failed install is not remembered, so retrying after a network error does real work again rather than handing back the earlier failure.
- **Writing a file the app manages no longer fails when a virus scanner happens to be holding it.** Config saves, the dependency installer and the first-run file copy all write to a temp file and then rename it over the destination, which is what makes them atomic and what lets them replace a hidden or read-only file. On Windows that rename is refused outright — reported as `EPERM` — for as long as any other process has the file open, and a real-time scanner opening the temp file a microsecond after it appears is the common way that happens. The rename is now retried, briefly and a bounded number of times, which is the only thing that helps: the operation being refused *is* the rename, so there is nothing to route around. A genuine permission error still fails, and still fails promptly. This was surfaced by three tests that had been failing unpredictably for weeks and had been put down to test-suite pollution; the cause was this, in code every install runs.
...
- **A pairing attempt you replace by starting another one now says so.** Starting a second pairing attempt drops the first, and the first attempt's page was left polling an id the server no longer recognised — so it reported "That pairing session is no longer available.", which describes a failed lookup rather than what happened. It now reports that it was replaced, and points at the attempt that replaced it. Cancelling is unchanged and still ends an attempt outright.

## [0.1.30-beta.127] - 2026-09-16

### Added

- **Wireless pairing, by QR code or by typed pairing code.** Android 11+ devices could not be connected over the network at all without it — wireless debugging on those devices listens on a TLS-secured port that refuses an unpaired client outright, so a scan or a manual `ip:port` add found the device and then failed to connect, every time. The natural next question — "why didn't the phone show a pairing prompt?" — has no answer to find, because Android 11+ never shows one: pairing is not something the *device* offers a connecting client, it is something the client has to initiate, using the pairing screen under Settings → Developer options → Wireless debugging. The failure surfaces as a plain `failed to connect`, indistinguishable from an unreachable host unless you already know the device needs pairing first. Home's **Available Network Devices** panel now offers a **"Pair a new device"** section, beside **scan network** and **manually add**: **scan QR code** puts a QR *on the app's own screen*, for the phone's wireless-debugging pairing scanner to read (advertising over `_adb-tls-pairing._tcp`) — the phone scans the app, never the other way round — and **pairing code** takes the `ip:port` and six-digit code the phone displays instead. Either path pairs the device and then auto-connects it — the QR flow discovers the device by polling for the service name it embedded in the code; the typed-code flow already has the address. A device that paired but could not be auto-connected is reported as such rather than as a failure — the trust relationship is durable even when the last step needs finishing by hand, and the app offers a direct **connect** button when it knows the address; when it never found one at all it opens **manually add** for you — empty, including the port, because there is nothing to fill it from and the usual `5555` default is the one port a TLS device is known not to listen on — and tells you to type the `ip:port` from the phone's own Wireless debugging screen, not the different, one-shot port on the pairing dialog. Pairing is **admin-only**, deliberately stricter than connecting: it establishes a persistent trust relationship with a new device on behalf of the whole server, so a signed-in non-admin still sees the section but is told to ask an administrator. Smoke rows **7.6** (QR) and **7.7** (typed code — the only path that can pair a device with no camera) cover it.
