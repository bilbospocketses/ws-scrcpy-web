...
- **Wireless pairing, by QR code or by typed pairing code.** Android 11+ devices could not be connected over the network at all without it — wireless debugging on those devices listens on a TLS-secured port that refuses an unpaired client outright, so a scan or a manual `ip:port` add found the device and then failed to connect, every time. The natural next question — "why didn't the phone show a pairing prompt?" — has no answer to find, because Android 11+ never shows one: pairing is not something the *device* offers a connecting client, it is something the client has to initiate, using the pairing screen under Settings → Developer options → Wireless debugging. The failure surfaces as a plain `failed to connect`, indistinguishable from an unreachable host unless you already know the device needs pairing first. Home now offers **scan a QR code** (Settings shows a QR the phone's wireless-debugging pairing scanner reads directly, advertising over `_adb-tls-pairing._tcp`) and **enter a pairing code** (type the `ip:port` and six-digit code the phone displays) side by side in the existing scan/connect modal. Either path pairs the device and then auto-connects it — the QR flow discovers the device by polling for the service name it embedded in the code; the typed-code flow already has the address. A device that paired but could not be auto-connected (no matching connect service was found) is reported as such rather than as a failure — the trust relationship is durable even when the last step needs finishing by hand. Smoke rows **7.6** (QR) and **7.7** (typed code — the only path that can pair a device with no camera) cover it.
