...
- **First-run modal dismissal survives port shifts.** On page load, server-side `firstRunComplete` / `serviceFirstRunSeen` flags (already dual-written when modals are dismissed) are now synced into localStorage before the modal gate check runs. Previously, navigating to a new port (different origin) cleared localStorage flags, causing dismissed modals to re-fire.
