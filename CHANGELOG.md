...
- Device card overlay buttons laid out as a 2×2 grid: row 1 `[shell][list files]`, row 2 `[configure stream][connect]` (chunkier buttons at the bottom reads more balanced than at the top). `.services` swapped from `flex-wrap` to `display: grid; grid-template-columns: 1fr 1fr; place-items: center`; the `.services-break` hack div is gone. `.services-label` spans both columns via `grid-column: 1 / -1`
