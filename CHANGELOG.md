...
- **The update channel and the update check interval no longer save on the spot.** Each used to fire its own write the moment you touched it, so flipping one and closing the dialog saved it. They are now staged like the web port and applied by Save — closing without saving changes nothing. The GitHub-owner field is the exception and still writes as soon as you leave it. Buttons were never settings and are unchanged: installing the service, adding a user, installing a dependency and the rest all act immediately when clicked and never appear on the review screen.
