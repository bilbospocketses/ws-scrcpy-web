...
- **Wireless pairing, by QR code or by typed pairing code.** Android 11+ devices could not be connected over the network at all without it — wireless debugging on those devices listens on a TLS-secured port that refuses an unpaired client outright, so a scan or a manual `ip:port` add found the device and then failed to connect, every time. The natural next question — "why didn't the phone show a pairing prompt?" — has no answer to find, because Android 11+ never shows one: pairing is not something the *device* offers a connecting client, it is something the client has to initiate, using the pairing screen under Settings → Developer options → Wireless debugging. The failure surfaces as a plain `failed to connect`, indistinguishable from an unreachable host unless you already know the device needs pairing first. Home's **Available Network Devices** panel now offers a **"Pair a new device"** section, beside **scan network** and **manually add**: **scan QR code** puts a QR *on the app's own screen*, for the phone's wireless-debugging pairing scanner to read (advertising over `_adb-tls-pairing._tcp`) — the phone scans the app, never the other way round — and **pairing code** takes the `ip:port` and six-digit code the phone displays instead. Either path pairs the device and then auto-connects it — the QR flow discovers the device by polling for the service name it embedded in the code; the typed-code flow already has the address. A device that paired but could not be auto-connected is reported as such rather than as a failure — the trust relationship is durable even when the last step needs finishing by hand, and the app offers a direct **connect** button or, address permitting, opens **manually add** pre-filled. Smoke rows **7.6** (QR) and **7.7** (typed code — the only path that can pair a device with no camera) cover it.
