...
fn write_post_stop_bat(
    ...
    //
    // §32 Part 5b (post-smoke fix): the upgrade-server is now spawned
    // pre-exit from UpdateService.applyUpdate (Node side), NOT from this
    // bat. Spawning pre-exit + retry-binding closes the dead window between
    // Node port-release and upgrade-server bind that left the browser on
    // "can't be reached" through the entire upgrade window in the beta.20
    // smoke. The bat retains only the marker-gated sc-start sequence;
    // recovery of the service is still its job, but presenting the
    // updating page is no longer.
    //
    ...
    let bat = format!(
        "@echo off\r\n\
         REM ws-scrcpy-web post-stop handler (§32 Part 5b).\r\n\
         ...
         REM Upgrade-server (updating-page) is spawned pre-exit by UpdateService.applyUpdate,\r\n\
         REM not by this bat — see §32 Part 5b in todo_ws_scrcpy_web.md.\r\n\
         if exist \"{marker}\" (\r\n\
