...
6. **Commit and tag.**
   ...
   ```
7. **Watch CI.** The tag push triggers `.github/workflows/release.yml`. Both Windows and Linux jobs run in parallel; the `publish` job uploads artifacts (Setup.exe, portable ZIP, AppImage, AppImage.sig once SignPath is live, nupkg, releases.stable.json, SHA256SUMS) and creates a GitHub Release with auto-generated notes.
   ```bash
   gh run watch
   ```
...
2. **Add the four non-secret identifiers** as repository variables (NOT secrets -- they're visible in the workflow runs):
   ...
   - `SIGNPATH_WINDOWS_POLICY_SLUG` -- the slug for the policy that signs Windows EXEs and Setup.exe (one Authenticode policy covers all)
