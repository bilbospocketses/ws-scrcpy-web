...
**Automated today: 87 of 155 = 56 %.** (86 / 55 % until 8.17 moved to the device tier on qa-harness #95,
...
automated, **55 %** later that day, once item 148 counted the Windows guest tier
P4 had already been running, and **56 %** once 8.17 was automated on qa-harness #95. That figure counts rows the way item 13 did,
...
read. Four were harness gaps, and qa-harness closed them in #98 (2026-09-25): 1.6
passed vacuously on the default port, 1.10's ACE regex accepted an inherited ACE,
...
| 1.6 | `[Win]` | Reinstall reuses config | windows guest | qa-harness P4, `arcs/01c-uninstall.ps1`: `config.json`'s `webPort` is identical across the reinstall, the port is first moved off the skeleton default if needed (so 8000 → 8000 cannot pass vacuously), and the app is relaunched and must answer HTTP 200 on that saved port (qa-harness #98; on beta.134 it reused 8002 and answered). |
...
| 1.10 | `[Win]` | Install-dir ACL grant (install hook; no UAC on a fresh install) | windows guest | qa-harness P4, `arcs/01a-install.ps1`, in Prompt mode with the consent observer polling through both launches: zero UAC, an EXPLICIT `Authenticated Users:(OI)(CI)(M)` ACE (an inherited `(I)` ACE no longer passes, qa-harness #98), the hook line, no fallback line. |
...
| 5.10 | `[Win]` | Service install, UAC declined | manual/conditional | qa-harness P4, `arcs/01c-uninstall.ps1` + `service.spec.ts`. **Needs a person:** the consent dialog exposes no UI Automation tree, so nothing can click No, and an unattended run Skips by design. With someone at the prompt it asserts the declined message, the freed button and no service. Reworded 2026-09-25 from an uninstall decline that Windows cannot produce. **First attended pass on beta.136** (qa-harness #100, run `win-20260926T041300Z`): one consent dialog, the operator clicked No, nothing was left behind. The app's 480 s wait is the only deadline; Windows leaves an unanswered prompt up (measured at 10.5 min). |
...
| 8.16 | `[Both]` | An unavailable audio codec no longer kills the video stream | automatable: no spec yet | Device tier, and the fixture is specific: the device must LACK the requested codec. x86_64 redroid lacked Opus when measured on 2026-09-23, but only because it was on its **legacy OMX stack**, which it falls back to when the guest kernel has no `/dev/dma_heap/system`. On a kernel with it (stock Ubuntu 24.04), redroid runs Codec2 and lists `c2.android.opus.encoder`, so the fallback never fires, and a real phone usually has all three. **qa-harness will not automate it** (user decision 2026-09-25): it is compatibility coverage for legacy or stripped devices, and `audioCodecFallback.test.ts` carries it. It stays here because an OMX-stack fixture would make it automatable, not because anything is missing from the product. |
| 8.17 | `[Both]` | A device that never sends codec config is recovered, not left black | device | qa-harness virtual-device tier: `suites/wssw-virtual/playwright/specs/config-recovery.spec.ts` (`@redroid`, profile `redroid`: redroid 13 x86_64 on its Codec2 stack, `c2.android.avc.encoder`, the #703 reporter's own encoder; qa-harness #95, merged 2026-09-25). **It reproduces on Codec2:** on beta.132, 13 of 24 sessions across three runs stalled with media frames arriving and no config, and all 24 recovered. The spec reads the stall from the server's own `stream summary` counters (first frame before first config, or config never), asserts every session's encoder is `c2.android.avc.encoder` and that recovery brings a keyframe, and records which mechanism recovered each session: on this device it was always the BROWSER's decode watchdog (5 s), which fires before the server's (8 s); see TECHNICAL_GUIDE 25.9. The 2026-09-23 counts (5 of 8 on x86_64, 6 of 6 on emulated arm64) were from redroid's legacy OMX stack. The run is probabilistic, so a single green session proves nothing. Since qa-harness #99 the spec also reads the browser console: `First decoded frame` is the picture witness, and a session counts as client-reset only when the console shows the request. On beta.136 (qa-harness #100): 2 of 8 stalled, both recovered by the browser. |
...
| 15.5 | `[Win]` | Server-section order | windows guest | `readonly.spec.ts`, run in qa-harness P4's Arc 1a: the VISIBLE row labels in order (reset all my settings → web port → stop the server and close the app → uninstall ws-scrcpy-web), the visible buttons as a second witness, and "install for all users" built in the DOM but not visible, so the check can fail both ways (qa-harness #98). |
