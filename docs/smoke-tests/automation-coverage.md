...
| Automatable, no spec written yet | 7 | — (**15.6**, the tray's Exit end to end, which the Windows guest tier can reach once it can right-click a real tray icon (qa-harness item 21) but P4 does not claim; **20.4 / 20.5**, the container install/uninstall rows unblocked 2026-09-04, which this line counted as automated container rows until 2026-09-25 although neither has a spec; 8.10 / 8.11, item 24's rotation rows; the six of 2026-09-04 were written 2026-09-06, item 104; **plus two of item 68's four beta.131 rows — 8.16 and 8.17** (8.15 and 12.6 were automated 2026-09-25), both measured 2026-09-23 on an x86_64 redroid that was on its **legacy OMX stack** (a guest kernel without `/dev/dma_heap/system`): it had **no Opus encoder at all** (8.16) and **withheld SPS/PPS in roughly 5 of 8 sessions** (8.17). On a kernel with dma_heap, redroid runs Codec2, which has Opus. So 8.16 needs an OMX fixture that qa-harness has decided not to build, and 8.17 is being measured on Codec2, where it is not yet known to reproduce) |
...
| 8.17 | `[Both]` | A device that never sends codec config is recovered, not left black | automatable: no spec yet | Device tier. **redroid 13 withheld SPS/PPS in roughly 5 of 8 sessions on x86_64 and 6 of 6 on emulated arm64** (measured 2026-09-23, issue #703), which is the only fixture known to reproduce it — so this is automatable but NOT on a healthy phone. The x86_64 count was on redroid's legacy OMX stack (a guest kernel without `/dev/dma_heap/system`). qa-harness's redroid tier runs Codec2 on `c2.android.avc.encoder`, the #703 reporter's own encoder, where it is not yet established: if 8 sessions never reproduce it, the row reports NOT REPRODUCED and never passes. Two traps for whoever writes it: the run is probabilistic, so a single green session proves nothing; and the capture window must exceed the device's own latency — a 14s window read a working fix as 6-of-6 broken because the emulated device needed ~20s. |
