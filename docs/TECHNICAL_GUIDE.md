...
| File | Source | Content |
|------|--------|---------|
| `launcher.log` | Rust launcher | Supervisor lifecycle, control markers, elevated runner, operation-server |
| `server.log` | Rust launcher (captured Node stdout/stderr) | Node child process console output |
| `ws-scrcpy-web.log` | Node `Logger` utility | Application-level logging (this section's subject) |
| `service.log` | Servy service manager | Service-mode stdio capture |
| `tray.log` | Tray helper (`ws-scrcpy-web-tray.exe`) | Tray lifecycle, per-session mutex, balloon notifications |
...
2. Capture Node stdout/stderr to `<dataRoot>/logs/server.log`
