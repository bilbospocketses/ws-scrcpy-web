...
- `WsScrcpyWeb.AppImage`, the launcher helper, and a copy of the user's `dependencies/` → labelled **`bin_t`** (persistent `semanage fcontext` + `restorecon`) so `init_t` may exec them, and so the service runs its **own** deps instead of reaching into a user's home.
